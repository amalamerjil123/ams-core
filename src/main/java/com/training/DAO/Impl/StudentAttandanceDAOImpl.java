...
public class StudentAttandanceDAOImpl implements StudentAttandanceRepository {
	...
	public List<StudentAttendanceDTO> getStudentAttendanceList(String userName) {
		...
		StringBuilder sql = new StringBuilder(
				"SELECT s.name,s.email,s.department,s.mobile_no,a.attended,a.id,a.reason,a.created_date,a.modified_date,"
						+ "c.name,s.batch_id,b.batch_name,t.training_date,u.`name` updatedBy,uu.`name` reviewedName,ss.status,a.present_by_student FROM attendance a JOIN students s ON "
						...
						+ "LEFT JOIN users uu ON a.`reviewed_by`=uu.id LEFT JOIN seed_status ss ON ss.`id`=a.`review_status` WHERE s.email=?");
	...
	public List<StudentAttendanceDTO> getStudentAttendanceInReviewList() {
		List<StudentAttendanceDTO> list = null;
		StringBuilder sql = new StringBuilder(
				"SELECT a.`id`,s.`name`,s.`email`,s.`department`,s.`mobile_no`,a.`reason`,c.`name`,a.present_by_student,t.`training_date` FROM attendance a JOIN students s"
...
public class StudentAttandanceDAOImpl implements StudentAttandanceRepository {
    ...
    public List<StudentAttendanceDTO> getStudentAttendanceList(String userName) {
        ...
        StringBuilder sql = new StringBuilder(
                "SELECT s.name,s.email,s.department,s.mobile_no,a.attended,a.id,a.reason,a.created_date,a.modified_date,"
                        + "c.name,s.batch_id,b.batch_name,t.training_date,u.`name` updatedBy,uu.`name` reviewedName,ss.status,a.present_by_student,t.freeze FROM attendance a JOIN students s ON "
                        + "a.student_id=s.id JOIN batches b ON b.batch_no=s.batch_id JOIN colleges c ON c.`id`=b.`college_id`"
                        + " JOIN training t ON a.training_id=t.id LEFT JOIN users u ON a.`modified_by`=u.`id` "
                        + "LEFT JOIN users uu ON a.`reviewed_by`=uu.id LEFT JOIN seed_status ss ON ss.`id`=a.`review_status` WHERE s.email=? ORDER BY t.training_date");
        list = jdbcTemplate.query(sql.toString(), new Object[] { userName }, (
                rs, rowNum) -> {
            return convertToStudentObj(rs);
        });
    ...
    public List<StudentAttendanceDTO> getStudentAttendanceInReviewList() {
        List<StudentAttendanceDTO> list = null;
        StringBuilder sql = new StringBuilder(
                "SELECT a.`id`,s.`name`,s.`email`,s.`department`,s.`mobile_no`,a.`reason`,c.`name`,a.present_by_student,t.`training_date`,a.present_by_student FROM attendance a JOIN students s"
                        + " ON s.`id`=a.`student_id` LEFT JOIN `batches` b ON s.`batch_id`=b.`batch_no` LEFT JOIN `colleges` c "
                        + "ON b.`college_id`=c.`id` LEFT JOIN training t ON a.`training_id`=t.`id` WHERE a.`review_status`=1");
        list = jdbcTemplate.query(sql.toString(), new Object[] {},
                (rs, rowNum) -> {
